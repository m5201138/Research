...
#define Distance_h
struct Distance {
    typedef PointForTree Query_item;
    typedef double FT;
    typedef CGAL::Dimension_tag<3> D;
    double transformed_distance(const PointForTree& p1, const PointForTree& p2) const {
        double distx= p1.x()-p2.x();
        double disty= p1.y()-p2.y();
        double distz= p1.z()-p2.z();
        return distx*distx+disty*disty+distz*distz;
    }
    double min_distance_to_rectangle(const PointForTree& p,
    ...
    double max_distance_to_rectangle(const PointForTree& p,
                                     const CGAL::Kd_tree_rectangle<FT,D>& b) const {
        double h = p.x();
